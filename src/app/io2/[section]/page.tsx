...
const canonicalSectionIds = {
  ...
  "julian-opie": "julien-opie",
  fallingwater: "fallingwater",
  "final-work": "final-work",
} as const;

function getArchiveSection(section: string) {
  const archiveId = canonicalSectionIds[section as keyof typeof canonicalSectionIds];
  return archiveId ? io2ArchiveSectionMap.get(archiveId) : undefined;
}

export function generateStaticParams() {
  return Object.keys(canonicalSectionIds).map((section) => ({ section }));
}

export async function generateMetadata({ params }: IO2CanonicalPageProps) {
  const { section } = await params;
  const archiveSection = getArchiveSection(section);
...
export default async function IO2CanonicalPage({ params }: IO2CanonicalPageProps) {
  const { section } = await params;
  const archiveSection = getArchiveSection(section);
